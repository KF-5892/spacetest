...
import {
  ...
  RAIL_A, RAIL_B, SUBTITLES, TOUR_LENGTH, WORLD_SWAP_T, phaseAt,
} from './tour';
...
class App {
  ...
  private readonly railA = new Rail(RAIL_A);
  private readonly railB = new Rail(RAIL_B);
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.scene.add(this.worldA, this.worldB);
  ...
  private applyTour(dt: number): void {
    ...
    // 世界の切替(ワープの光に隠れて行う)
    const inB = t >= WORLD_SWAP_T;
    this.worldA.visible = !inB;
    this.worldB.visible = inB;

    // レール上の位置と注視点
    const rail = inB ? this.railB : this.railA;
    const u = rail.uAt(t);
    ...
    this.dust.points.visible = this.quality === 'high' && warpAmt < 0.3;

    // カメラ演出パラメータ(快適モードでは減衰: NFR-33)
    const comfortMul = this.comfort ? 0.4 : 1;
    const boardingKick = phase.id === 'boarding' ? clamp01(t / 6) * 3.5 : 0;
    const fovKick = (warpAmt * 20 + charge * 4 + boardingKick) * comfortMul;
    const rumble = (charge * 0.14 + warpAmt * 0.28) * (this.comfort ? 0.35 : 1);
    const swayAmp = this.comfort ? 0.25 : 1;
    ...
    // 終幕: フェード→終了画面
    const fade = t > 95 ? clamp01((t - 95) / 2.5) : 0;
